...
MODEL_WEIGHTS = '/home/niravraham/Downloads/deeplab_resnet.ckpt'
NUM_CLASSES = 21
SAVE_DIR = './output/'
...
def apply_mask_all_classes(img_path, mask_path, save_dir):
    img = np.array(Image.open(img_path))
    mask = np.array(Image.open(mask_path))
    mask = mask[:, :, 0].astype(float) + mask[:, :, 1].astype(float) + mask[:, :, 2].astype(float)
    mask = [mask != 0] * np.ones(mask.shape, dtype=np.int)
    mask = np.squeeze(mask, axis=0)
    mask_img = np.zeros(img.shape)
    mask_img[:, :, 0] = img[:, :, 0] * mask
    mask_img[:, :, 1] = img[:, :, 1] * mask
    mask_img[:, :, 2] = img[:, :, 2] * mask
    im = Image.fromarray(mask_img.astype('uint8'))
    filename = os.path.basename(img_path).split('.')[0]
    im.save(save_dir + filename + '_result.jpeg')
...
def main():
    ...
    print('The output file has been saved to {}'.format(args.save_dir + filename + '_result.jpeg'))
